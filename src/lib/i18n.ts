...
const ja = {
  ocrEngine: 'OCRモデル',
  ocrEngineNdl: 'NDL古典籍OCR-Lite',
  ocrEngineHonkoku: 'みんなで翻刻OCR v19',
  ocrEngineNdlDetail: '軽量なPARSeq行認識。モデル初回約82MB。',
  ocrEngineHonkokuDetail: 'Koji記法対応。認識モデル初回約155MB（WASM）／約248MB（WebGPU）＋NDL行検出器。プロファイルは行検出とタイル処理に適用し、PARSeqの再認識は行いません。',
  ocrEngineUnavailable: 'この環境ではHonkokuモデルを利用できません。',
  ocrConfidenceUncalibrated: '未較正',
  ocrConfidenceUnavailable: '算出不可',
  documentTitle: "墨景 — 古典籍AI OCR",
  documentDescription: "国書データベースの書誌IDやIIIF Manifestから古典籍画像を読み込み、崩し字をOCRで読み取れるワークベンチです。",
  ...
  ocrConfidenceNote: "{engine} / {provider}。検出スコアと認識スコアを分けて表示しています。認識スコアは未較正の候補比較値で、文字ごとの正解確率ではありません。",
  generationScore: "生成スコア",
  generationShort: "生成",
  generationScoreHelp: "生成確率から求めた0〜100のスコアです。正解率ではなく、NDLの認識スコアとは直接比較できません。",
  generationLineScores: "生成 {generation}/100（正解率ではありません） / 検出 {detection}",
  ocrGenerationConfidenceNote: "{engine} / {provider}。生成スコアはモデルの生成確率に基づく0〜100の値です。正解率ではなく、NDLの認識スコアとは直接比較できません。",
  recognitionScore: "認識スコア",
  detectionScore: "検出スコア",
  recognitionShort: "認識",
  detectionShort: "検出",
  lineScores: "認識 {recognition} / 検出 {detection}",
  ocrTextSize: "OCR文字サイズ",
  ocrTextSizeDecrease: "OCR文字を小さくする",
  ocrTextSizeIncrease: "OCR文字を大きくする",
  ocrTextSizeValue: "{size}px",
  benchmarkExport: "OCR評価データを書き出す",
  exportJson: "JSON",
  exportCsv: "CSV",
  exportBaseline: "基準値JSON",
  ocrProfile: "処理プロファイル",
  ocrProfileFast: "高速",
  ocrProfileBalanced: "標準",
  ocrProfileAccurate: "高精度",
  benchmarkGroundTruth: "正解JSON（任意）",
  benchmarkEvaluate: "評価する",
  benchmarkGroundTruthRequired: "正解JSONを入力してください。",
  benchmarkMetrics: "評価結果",
  benchmarkCer: "CER",
  benchmarkNormalizedCer: "正規化CER",
  benchmarkExact: "完全一致行率",
  benchmarkRecall: "検出Recall",
  benchmarkF1: "検出F1",
  ocrDeviceDescription: "選択したOCRモデルを端末内で実行し、文字行の検出・翻刻・座標表示を行います。",
  ...
  licenseRestricted: "ライセンス上、翻刻利用ができない可能性があります",
  ...
  progressRetry: "低信頼行を追加認識中 {completed} / {total}",
  ...
  errorHonkokuUnavailable: "Honkoku v19が無効、またはモデルManifestが未設定です。",
  errorGeneric: "処理に失敗しました。",
  serviceNoticeTitle: "ご利用前に",
  serviceNoticeBodyJa: "このOCRは国立国会図書館(NDL)が作成した古典籍OCR-Lite、みんなで翻刻OCR v19（設定時）、CODHのMetomくずし字認識サービスを利用します。NDLモデルはCC BY 4.0、翻刻v19モデルはCC BY-SA 4.0で公開されています。各配布元の利用条件をご確認ください。",
  serviceNoticeBodyEn: "This is a simple OCR service that uses Koten OCR-Lite, developed by the National Diet Library (NDL), Minna de Honkoku OCR v19 (when configured), and CODH's Metom. NDL models use CC BY 4.0; Honkoku v19 weights use CC BY-SA 4.0. Please check each publisher's terms.",
  serviceNoticeAgree: "同意する",
  serviceNoticeStorageWarning: "同意を保存できないため、次回も表示されます。",
  languageSelector: "言語",
  english: "English",
  japanese: "日本語",
} as const;
...
const en: Record<keyof typeof ja, string> = {
  ocrEngine: 'OCR model',
  ocrEngineNdl: 'NDL Kotenseki OCR-Lite',
  ocrEngineHonkoku: 'Minna de Honkoku OCR v19',
  ocrEngineNdlDetail: 'Lightweight PARSeq line recognition. Initial models approximately 82 MB.',
  ocrEngineHonkokuDetail: 'Koji markup. Initial recognition models approximately 155 MB (WASM) / 248 MB (WebGPU), plus NDL line detector. Profile changes detection and tiling, not PARSeq retries.',
  ocrEngineUnavailable: 'Honkoku is unavailable in this build or browser.',
  ocrConfidenceUncalibrated: 'Uncalibrated',
  ocrConfidenceUnavailable: 'N/A',
  ...
  ocrConfidenceNote: "{engine} / {provider}. Detection and recognition scores are shown separately. Recognition is an uncalibrated candidate-comparison score, not a per-character probability.",
  generationScore: "Generation score",
  generationShort: "Gen.",
  generationScoreHelp: "A 0–100 score based on token generation probabilities. It is not accuracy and is not directly comparable with NDL recognition scores.",
  generationLineScores: "Generation {generation}/100 (not accuracy) / detection {detection}",
  ocrGenerationConfidenceNote: "{engine} / {provider}. Generation scores range from 0 to 100 and reflect model generation probabilities. They are not accuracy and are not directly comparable with NDL recognition scores.",
  recognitionScore: "Recognition score",
  detectionScore: "Detection score",
  recognitionShort: "Rec.",
  detectionShort: "Det.",
  lineScores: "Recognition {recognition} / detection {detection}",
  ocrTextSize: "OCR text size",
  ocrTextSizeDecrease: "Decrease OCR text size",
  ocrTextSizeIncrease: "Increase OCR text size",
  ocrTextSizeValue: "{size}px",
  benchmarkExport: "Export OCR benchmark data",
  exportJson: "JSON",
  exportCsv: "CSV",
  exportBaseline: "Baseline JSON",
  ocrProfile: "OCR profile",
  ocrProfileFast: "Fast",
  ocrProfileBalanced: "Balanced",
  ocrProfileAccurate: "Accurate",
  benchmarkGroundTruth: "Ground truth JSON (optional)",
  benchmarkEvaluate: "Evaluate",
  benchmarkGroundTruthRequired: "Enter ground truth JSON first.",
  benchmarkMetrics: "Evaluation metrics",
  benchmarkCer: "CER",
  benchmarkNormalizedCer: "Normalized CER",
  benchmarkExact: "Exact-line rate",
  benchmarkRecall: "Detection recall",
  benchmarkF1: "Detection F1",
  ocrDeviceDescription: "Runs the selected OCR model on this device to detect, transcribe, and position character lines.",
  ...
  licenseRestricted: "Transcription may not be permitted under this license.",
  manifestLink: "Manifest ↗",
  presentationApi: "IIIF PRESENTATION API",
  ...
  progressRetry: "Retrying low-confidence lines {completed} / {total}",
  ...
  errorHonkokuUnavailable: "Honkoku v19 is disabled or its model manifest is not configured.",
  errorGeneric: "The operation failed.",
  serviceNoticeTitle: "Before you begin",
  serviceNoticeBodyJa: "このOCRは国立国会図書館(NDL)が作成した古典籍OCR-Lite、みんなで翻刻OCR v19（設定時）、CODHのMetomくずし字認識サービスを利用します。NDLモデルはCC BY 4.0、翻刻v19モデルはCC BY-SA 4.0で公開されています。各配布元の利用条件をご確認ください。",
  serviceNoticeBodyEn: "This is a simple OCR service that uses Koten OCR-Lite, developed by the National Diet Library (NDL), Minna de Honkoku OCR v19 (when configured), and CODH's Metom. NDL models use CC BY 4.0; Honkoku v19 weights use CC BY-SA 4.0. Please check each publisher's terms.",
  serviceNoticeAgree: "I agree",
  serviceNoticeStorageWarning: "Your agreement could not be saved, so this notice will appear again next time.",
